...
def animate():
    ...
    port = METRICS_SOCKETS["metric2"]["port"]
        ...
        while True:
            ...
            if data:
                ...
                try:
                    ...
                    click_list[tmp_index] = unserialised_data[2]
                except Exception as exception:
                    aid_list.append(aid)
                    click_list.append(unserialised_data[2])
                finally:
                    assert len(click_list) == len(aid_list)
                    print(aid_list, click_list)
                    plt.xlabel("Identifiant des articles")
                    plt.ylabel("Cliques count")
                    plt.xticks(aid_list)
                    rect = plt.bar(aid_list, click_list, width=0.8, color="#ef9a9a", label="Click count bar")
                    autolabel(rect)
